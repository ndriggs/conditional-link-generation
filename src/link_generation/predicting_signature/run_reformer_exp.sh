...
#SBATCH --output=job_output/equal_reformer_%A_%a.out
#SBATCH --error=job_output/equal_reformer_%A_%a.err
#SBATCH --array=0
...
python -m link_generation.predicting_signature.predict_signature --model="reformer" --preprocessing=${data[$data_index]} \
        --classification=${classification[$classification_index]} --d_model=${d_model[$d_model_index]} \
        --nheads=${nheads[$nheads_index]} --num_layers=${n_layers[$n_layers_index]}